...
class Policy(nn.Module):
    ...
    def __init__(self, state_dim: int, action_dim: int, hidden_sizes: List[int], action_limit: float, action_std: float=1e-2,
                 hidden_activation: Type[nn.Module] = nn.ReLU, output_activation: Type[nn.Module] = nn.Tanh, single_network=True):
        super().__init__()
        ...
        if single_network:
            ...
            self.pi, self.V = self.__create_networks()
    ...
    def network_dim(self):
        if self.single_network:
            ...
            return sum([np.prod(p.shape) for p in self.pi.parameters()]), sum([np.prod(p.shape) for p in self.V.parameters()])
    ...
    def set_state(self, state):
        ...
        if network.get('single_network'):
            ...
            self.pi.load_state_dict(network['actor'])
            self.V.load_state_dict(network['critic'])
    ...
    def get_state(self):
        if self.single_network:
            network = {
                    'single_network': self.single_network,
